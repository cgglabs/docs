...
In this frame of reference, $i_d$ is called the direct axis current and is aligned with the rotor’s magnetic field (produced by the permanent magnets). $i_q$ is the quadrature axis current and is perpendicular to the rotor’s magnetic field.
...
On the other hand, the inner current loop $i_d$ we will always want to drive to zero. This is because any current on the $i_d$ axis would waste energy since it would not lead to the maximum amount of torque.
