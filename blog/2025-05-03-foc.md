...
Specifically, "easy" as we will see in FOC means that we command a specific current (which produces torque) and we are able to track it as a time-invariant DC signal. This allows us to use something as simple as PID control to control the motor, even though the phase currents themselves are *actually* time-varying sinusoidal signals.

That last point is worth repeating.

FOC allows us to turn the difficult problem of controlling 3 time-varying sinusoidal signals into an "easy" one of controlling a time-invariant DC signal.
